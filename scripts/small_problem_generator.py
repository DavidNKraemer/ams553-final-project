...
for site_dist_id, site_dist in enumerate(site_distributions):
    for drone_dist_id, drone_dist in enumerate(drone_distributions):
        ...
        opt_tour, opt_cost = find_optimal_team_tour(sites, num_drones)

        for run in range(num_runs):
            data_dict[id] += [site_dist_id, drone_dist_id, run, opt_tour, opt_cost]
...
data_df = pd.DataFrame.from_dict(
    ...
    orient='index',
    columns=[
        "site_dist_id", "drone_dist_id", "run", "opt_tour", "opt_cost",
